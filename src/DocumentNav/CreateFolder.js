...
export default function CreateFolder({
  ...
  const fetchFolders = async () => {
    try {
      ...
      setData({ ...result, contents: filteredContents });
...
{folderdata && folderdata.contents && (
       ...
       <Box mt={2}>
       <Button variant="contained"  onClick={createFolderAPI}  disabled={!selectedFolder}>
         Create & Exit
        </Button>
       </Box>
...
const FolderContents = ({ contents, setSelectedFolder, selectedFolder, parentPath = "", templateId }) => {
  ...
  return (
    <List component="nav" sx={{ paddingLeft: 2,cursor:'pointer' }}>
      {contents.map((item, index) => {
